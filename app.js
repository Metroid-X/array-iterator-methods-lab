...
function checkVeryOld(o){
    return (1500 <= o.year && o.year <= 1599); 
}
veryOldInventors=inventors.filter(checkVeryOld)
...
function firstLastSwitch(s){
    let tmpArr = s.split(', ');
    let retStr = tmpArr[1]+' '+tmpArr[0];
    return retStr;
}
firstLast=people.map(firstLastSwitch);
...
function checkForAdult(q){
    return 2024-q.year >= 18;
}
isAdultPresent=devs.some(checkForAdult)
...
function checkFor19Up(q){
    return 2024-q.year >= 19;
}
isEveryone19OrOlder=devs.every(checkFor19Up)
...
function findBy823423(o){
    return o.id === 823423;
}
commentById=comments.find(findBy823423);
...
function findIdxBy123523(o){
    return o.id===123523;
}
idx=comments.findIndex(findIdxBy123523);
